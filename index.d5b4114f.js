...
window.progress = new (0, _src.Progress)({
    height: 5
});
window.delay = (ms)=>new Promise((res)=>setTimeout(res, ms));

},{"../src":"hngju"}],"hngju":[function(require,module,exports) {
...
class Progress {
        ...
        switch(this._state){
            ...
            case STATE.DISAPPEAR_RESTART:
                if (immediately) {
                    ...
                    this._timerId = null;
                    detach(this._el);
                } else this._state = STATE.DISAPPEAR;
                return;
        }
        if (immediately) {
            this._state = STATE.NOTHING;
    ...
    promise(p, { delay = 0, min = 100, waitAnimation = false } = {}) {
        let delayTid;
        const start = ()=>{
        ...
        if (delay > 0) this._delayTimers.push(delayTid = setTimeout(()=>{
            cleanupDelayTimer();
            start();
        }, delay));
        else start();
